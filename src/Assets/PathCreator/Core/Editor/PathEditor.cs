...
namespace PathCreationEditor {
    ...
    public class PathEditor : Editor {
                ...
                if (data.showPathOptions) {
                    ...
                    if (data.showTransformTool & (handleIndexToDisplayAsTransform == -1)) {
                        if (GUILayout.Button ("Centre Transform")) {
                            ...
                            if ( ( worldCentre - creator.transform.position ).sqrMagnitude > 0.01f ) {
        ...
        void SetTransformState (bool initialize = false) {
            ...
            if (!initialize) {
                if ( ( transformPos - t.position ).sqrMagnitude > 0.01f || 
                     ( t.localScale - transformScale ).sqrMagnitude > 0.01f || 
                     ( t.rotation.eulerAngles - transformRot.eulerAngles ).sqrMagnitude > 0.01f ) {
